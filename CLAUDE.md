...
- **The API base URL comes from `apps/menu/src/utils/api.ts`** (`API_BASE`, from `VITE_API_URL`, defaulting to `http://localhost:4000`). `CheckoutModal.tsx` uses it for `/api/orders`, `/api/payment/aba/create` and `/api/payment/aba/status/:orderId`. Other components (e.g. `OrdersView.tsx`) still hardcode `http://localhost:4000` and need the same treatment before a deploy.
