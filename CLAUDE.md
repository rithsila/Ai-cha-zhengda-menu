...
- **ABA PayWay is real, not a mock.** The API creates an ABA KHQR/deeplink payment at
  `POST /api/payment/aba/create` and confirms it at
  `GET /api/payment/aba/status/:orderId`. Both return **503** when `ABA_MERCHANT_ID` /
  `ABA_API_KEY` are unset — the server never substitutes fake credentials, because that
  turns a config mistake into an unreadable "wrong hash" error. The purchase uses
  `payment_option=abapay_khqr_deeplink`, which returns a mobile-app deeplink plus ABA's
  ready-to-render QR image. The checkout screen polls the **status** route, not the order
  row: only ABA's server-side `checkStatus` response and its amount check can mark an order
  paid. See `https://github.com/rithsila/aba-payway-unofficial/blob/main/docs/agent-guide.md`.
