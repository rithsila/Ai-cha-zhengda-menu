...
describe('StoreSettings strict layer', () => {
  ...
  it('updates Lucky Draw & Ticket Rules and saves to /api/config', async () => {
    render(
      <ToastProvider>
        <StoreSettings />
      </ToastProvider>
    ...
    expect(luckySwitch.getAttribute('aria-checked')).toBe('false');

    // Update tickets inputs
    const goldInput = screen.getByLabelText(/Tickets per Gold VIP Order/i);
    fireEvent.change(goldInput, { target: { value: '4' } });

    const stdInput = screen.getByLabelText(/Tickets per Standard Order/i);
    fireEvent.change(stdInput, { target: { value: '2' } });

    const spinCostInput = screen.getByLabelText(/Ticket Cost Per Lucky Spin/i);
    fireEvent.change(spinCostInput, { target: { value: '10' } });
